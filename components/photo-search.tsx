...
export function PhotoSearch() {
  ...
  const [pages, setPages] = useState<PexelsPhoto[][]>([]);
  ...
  const buildParams = useCallback(
    (targetPage: number, forQuery: string) => {
      ...
      params.set("query", forQuery || "photography");
  ...
  const search = useCallback(
    (forQuery: string) => {
      ...
      setPage(1);
      setLoading(true);
      fetch(`/api/search/photos?${buildParams(1, forQuery).toString()}`)
        .then((res) => res.json())
        .then((data: { photos: PexelsPhoto[] }) => setPages([data.photos]))
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !loading) {
          ...
          fetch(`/api/search/photos?${buildParams(nextPage, activeQuery).toString()}`)
            .then((res) => res.json())
    ...
    <div>
      <form onSubmit={runSearch} className="mx-auto mb-8 flex max-w-3xl flex-col gap-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar fotos… (ej. montañas, café, ciudad)"
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          <FilterDialog label="Color" options={COLORS} value={color} onChange={setColor} />
      ...
      {activeQuery === null ? (
        <p className="py-16 text-center text-sm text-zinc-500">
          Escribe algo y pulsa buscar para encontrar fotos.
        </p>
      ...
      {loading && (
        <p className="py-6 text-center text-sm text-zinc-500">Buscando…</p>
      )}
