...
export function VideoGallery({ initialVideos }: { initialVideos: PexelsVideo[] }) {
  const [pages, setPages] = useState<PexelsVideo[][]>([initialVideos]);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !loading) {
          ...
          fetch(`/api/videos?page=${nextPage}`)
            .then((res) => res.json())
  ...
  return (
    <div>
      ...
      {loading && (
        <p className="py-6 text-center text-sm text-zinc-500">Cargando más videos…</p>
      )}
