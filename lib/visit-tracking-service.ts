...
const ID_SEG_KEY = "galleryz_id_seg";
const HEARTBEAT_INTERVAL_MS = 60000;
const HUMAN_SIGNAL_TIMEOUT_MS = 12000;
const BOT_UA_REGEX =
  /bot|crawl|spider|slurp|bingpreview|headless|wget|curl|python-requests|aiohttp|httpclient|scanner|nikto|sqlmap|nmap/i;
  ...
  return new Promise((resolve) => {
    ...
    function cleanup(): void {
      HUMAN_SIGNAL_EVENTS.forEach((event) =>
        window.removeEventListener(event, onHumanAction, true),
      );
    }
    ...
    HUMAN_SIGNAL_EVENTS.forEach((event) =>
      window.addEventListener(event, onHumanAction, {
        once: true,
        passive: true,
        capture: true,
      }),
...
function registerVisit(): void {
  fetch(`${GATEWAY_BASE}/api/addgalleryz`, {
    method: "POST",
    mode: "cors",
    credentials: "omit",
    cache: "no-store",
...
function sendHeartbeat(): void {
  fetch(`${GATEWAY_BASE}/api/addkeepalivegalleryz?idSeg=${encodeURIComponent(getIdSeg())}`, {
    method: "POST",
    mode: "cors",
    credentials: "omit",
    cache: "no-store",
